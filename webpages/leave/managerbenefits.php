...
if (empty($accumulation['LEAVE']))
$accumulation['LEAVE'] = "0";
if (empty($accumulation['LEAVETAKEN']))
$accumulation['LEAVETAKEN'] = "0";
if (empty($accumulation['SICKLEAVE']))
$accumulation['SICKLEAVE'] = "0";
if (empty($accumulation['SICKLEAVETAKEN']))
$accumulation['SICKLEAVETAKEN'] = "0";
...
$emprequest = GrabAllData("SELECT * FROM HOLIDAYREQUEST WHERE STATUS = 'Pending' ORDER BY HOLREQID DESC");

$empreqidreq = $emprequest['HOLREQID'];
$empempidreq = $emprequest['EMPID'];
$empstartdatereq = $emprequest['STARTDATE'];
$empenddatereq = $emprequest['ENDDATE'];
$empstatusreq = $emprequest['STATUS'];
...
$holileft = $accumulation['LEAVE'] - ($datetotal * 8);
...
if(!empty($_POST['approvebutton']))
{
    ...
    $rowdelete = InsertData("UPDATE HOLIDAYREQUEST SET STATUS = 'Approved' WHERE HOLREQID= :REQID", array(array(":REQID", $approvebutton)));
}	

if(!empty($_POST['denybutton']))
{
    $denybutton = $_POST['denybutton'];
    
    $rowdelete = InsertData("UPDATE HOLIDAYREQUEST SET STATUS = 'Denied' WHERE HOLREQID= :REQID", array(array(":REQID", $denybutton)));
...
<head>
	...
	<link rel="stylesheet" href="managerbenefits.css">
...
<body class="annualBody">
	...
	<div class="benefitsContainersOutsideDiv">
		...
		<div class="infoCont">
			<div class="holidaystitle annualTitles">Leave left (hrs)</div>
				<div class="holidaynumber annualNumber"><?php echo $accumulation['LEAVE'] - ($datetotal * 8);?></div>
		</div>
		<div class="infoCont ">
    ...
    <a style="margin-top:40px;">Pending Requests</a>    
	   ...
	   <tbody>
				...
				FOR($i = 0; $i < count ($holreqid); $i++){
                    ...
                    "<tr class='base'> 
                       ...
                       <td>";
                    if ($holreqstartdate[$i] == null){echo "N/A";}                
                            else {echo date('d/m/Y', $holreqstartdate[$i]);}
                    echo "</td>
    ...
    <a style="margin-top:40px;">Employee Requests</a>    
	...
	<div id="holidayhistorytable">
      ...
      <thead>
	   ...
	   <tbody>
				<?php				
				FOR($i = 0; $i < count ($empreqidreq); $i++){
                    echo 
                    "<tr class='base'> 
					   <td class='holereqtd'>" . $empreqidreq[$i] . "</td>
                       <td class='holreqempidid'>" . $empempidreq[$i] . "</td>
                       <td>" . date('d/m/Y', $empstartdatereq[$i]) . "</td>
                       <td>" . date('d/m/Y', $empenddatereq[$i]) . "</td>
                       <td>" . $empstatusreq[$i] . "</td>
                       <td class='approvebtn'><form class='approveform' id='approveform$i' name='$i' action='#' method='post'><input type='hidden' value='$empreqidreq[$i]' name='approvebutton'><input type='submit' class='buttonapprove' value='Approve'></form></td>
                       <td class='denybtn'><form class='denyform' id='denyform$i' name='$i' action='#' method='post'><input type='hidden' value='$empreqidreq[$i]' name='denybutton'><input type='submit' class='buttondeny' value='Deny'></form></td>"                        ;
				};
            echo  "</tr>"; 
				 ?>       
	   </tbody>
...
<script>
    ...
    $(document).ready(function(){
        $(".buttondelete").on("click", function(event){
            event.preventDefault();
            form = $(this).parent();
            submitForm(form, "Are you sure you want to cancel this request?");
        });
        $(".buttonapprove").on("click", function(event){
            event.preventDefault();
            form = $(this).parent();
            submitForm(form, "Are you sure you want to approve this request?");
        });

        $(".buttondeny").on("click", function(event){
            event.preventDefault();
            form = $(this).parent();   
            submitForm(form, "Are you sure you want to deny this request?");
        });
    })    
    ...
    function submitForm(id, message) {
        ...
        parent.showPopUp(message,"Working, please wait...", function(){
